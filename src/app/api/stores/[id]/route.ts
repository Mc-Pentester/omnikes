import { NextRequest, NextResponse } from 'next/server';
import { storeService } from '@omnikes/services/store.service';
import { storeRepository } from '@omnikes/repositories/store.repository';
import { requireCurrentOrganizationId, requirePermission } from '@omnikes/lib/auth';
import { storeSchema } from '@omnikes/lib/validation';

/**
 * GET /api/stores/[id]
 * Get a store by ID for the authenticated user's organization
 */
export async function GET(
  request: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  try {
    const organizationId = await requireCurrentOrganizationId(request);
    const { id: storeId } = await params;

    const store = await storeService.getStore(storeId, organizationId);

    return NextResponse.json({ store });
  } catch (error) {
    if (error instanceof Error) {
      if (error.message === 'Authentication required' || error.message === 'Invalid or expired session') {
        return NextResponse.json(
          { error: 'Authentication required' },
          { status: 401 }
        );
      }

      if (error.message === 'Store not found or access denied') {
        return NextResponse.json(
          { error: 'Store not found or access denied' },
          { status: 404 }
        );
      }
    }
    
    console.error('Error getting store:', error);
    return NextResponse.json(
      { error: 'Failed to get store' },
      { status: 500 }
    );
  }
}

/**
 * PATCH /api/stores/[id]
 * Update a store for the authenticated user's organization
 */
export async function PATCH(
  request: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  try {
    const organizationId = await requireCurrentOrganizationId(request);
    await requirePermission(request, 'store.update');
    const { id: storeId } = await params;
    const body = await request.json();

    // Validate input using Zod schema (partial, exclude organizationId)
    const validatedData = storeSchema.partial().omit({ organizationId: true }).parse(body);

    // Check for duplicate code within the same organization (if code is being updated)
    if (validatedData.code) {
      const existingStore = await storeRepository.findByCode(validatedData.code, organizationId);
      if (existingStore && existingStore.id !== storeId) {
        return NextResponse.json(
          { error: 'Un magasin avec ce code existe déjà dans cette organisation.' },
          { status: 409 }
        );
      }
    }

    // Update store (organizationId cannot be changed)
    const store = await storeService.update(storeId, organizationId, validatedData);

    return NextResponse.json({ store });
  } catch (error) {
    if (error instanceof Error) {
      if (error.message === 'Authentication required' || error.message === 'Invalid or expired session') {
        return NextResponse.json(
          { error: 'Authentication required' },
          { status: 401 }
        );
      }

      if (error.message.startsWith('Permission required')) {
        return NextResponse.json(
          { error: 'Permission required' },
          { status: 403 }
        );
      }

      if (error.message === 'Store not found or access denied') {
        return NextResponse.json(
          { error: 'Store not found or access denied' },
          { status: 404 }
        );
      }

      // Zod validation errors
      if (error.name === 'ZodError') {
        return NextResponse.json(
          { error: 'Invalid input data', details: error.message },
          { status: 400 }
        );
      }
    }
    
    console.error('Error updating store:', error);
    return NextResponse.json(
      { error: 'Failed to update store' },
      { status: 500 }
    );
  }
}

/**
 * DELETE /api/stores/[id]
 * Deactivate a store (soft delete)
 */
export async function DELETE(
  request: NextRequest,
  { params }: { params: Promise<{ id: string }> }
) {
  try {
    const organizationId = await requireCurrentOrganizationId(request);
    await requirePermission(request, 'store.delete');
    const { id: storeId } = await params;

    const store = await storeService.deactivate(storeId, organizationId);

    return NextResponse.json({ store });
  } catch (error) {
    if (error instanceof Error) {
      if (error.message === 'Authentication required' || error.message === 'Invalid or expired session') {
        return NextResponse.json(
          { error: 'Authentication required' },
          { status: 401 }
        );
      }

      if (error.message.startsWith('Permission required')) {
        return NextResponse.json(
          { error: 'Permission required' },
          { status: 403 }
        );
      }

      if (error.message === 'Store not found or access denied') {
        return NextResponse.json(
          { error: 'Store not found or access denied' },
          { status: 404 }
        );
      }
    }
    
    console.error('Error deactivating store:', error);
    return NextResponse.json(
      { error: 'Failed to deactivate store' },
      { status: 500 }
    );
  }
}
